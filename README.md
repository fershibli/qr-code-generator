...
**Upload logo** accepts an image. Logo size (10–40%) and logo padding (0–25%) appear only when a file is selected. Recent logos stay under the upload control for reuse.
...
Use the sun/moon switch in the header. The choice is stored in `localStorage` (`color-mode`). The QR preview background stays white so the code stays scannable.

![The same generator in dark mode](docs/screenshots/dark-mode.png)

## Architecture

The UI is a single page. `App` owns form state, debounces generation, and hands a blob to the preview for download. QR drawing is client-side: `qrcode` paints a canvas, an optional logo is composited in the center, and a custom encoder writes an RGB PNG (color type 2) via `fflate`.
...
| QR rendering | `qrcode` on canvas, then a custom RGB PNG encoder (`fflate`) |
...
- Custom module and background colors, with a check that contrast stays scannable.
